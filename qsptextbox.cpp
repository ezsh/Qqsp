...
// The meaning of foreground colour varies according to the window class; it may be the text colour or other colour, or it may not be used at all. Additionally,
// not all native controls support changing their foreground colour so this method may change their colour only partially or even not at all.
...
QVariant QspTextBox::loadResource(int type, const QUrl& name)
{
	...
	if (new_name.endsWith(".mp4", Qt::CaseInsensitive) || new_name.endsWith(".avi", Qt::CaseInsensitive) || new_name.endsWith(".wmv", Qt::CaseInsensitive) ||
	    new_name.endsWith(".mkv", Qt::CaseInsensitive) || new_name.endsWith(".webm", Qt::CaseInsensitive) || new_name.endsWith(".m4v", Qt::CaseInsensitive) ||
							...
							if (animations_gif.contains(it.fragment().charFormat().toImageFormat().name())) {
								...
								if (it.fragment().charFormat().toImageFormat().width() > 0 && it.fragment().charFormat().toImageFormat().height() > 0) {
									animations_gif[it.fragment().charFormat().toImageFormat().name()].movieLabel->setGeometry(
										curRect.x(),
										curRect.y(),
										it.fragment().charFormat().toImageFormat().width(),
									...
									if (animations_video[it.fragment().charFormat().toImageFormat().name()].videoLabel->hasFrame()) {
										...
										if (it.fragment().charFormat().toImageFormat().width() > 0 && it.fragment().charFormat().toImageFormat().height() > 0) {
											...
											if (!animations_video[it.fragment().charFormat().toImageFormat().name()].videoLabel->resolution_set) {
												...
												newImageFormat.setWidth(
													animations_video[it.fragment().charFormat().toImageFormat().name()].videoLabel->getResolution().width());
												newImageFormat.setHeight(
													animations_video[it.fragment().charFormat().toImageFormat().name()].videoLabel->getResolution().height());
