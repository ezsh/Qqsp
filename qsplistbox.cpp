...
void QspListBox::createList()
{
	...
	for (int i = 0; i < qMin(m_images.size(), m_descs.size()); i++) {
		...
		QSize sizehint = QSize(
			this->width() - style()->pixelMetric(QStyle::PM_ScrollBarExtent) - 4 - item_widget->frameWidth() * 4,
...
QString QspListBox::formatItem(int itemIndex)
{
	...
	if (m_isShowNums && itemIndex < 9) {
		if (isImage) {
			return QString(
					   "<div style=\"color : #%1; -qt-block-indent:0; text-indent:0px;\"><div style=\"padding:0px; margin-right:4px;\">[%2]</div><div "
					   "style=\"padding:0px; margin-right:4px;\"><img src=\"%3\"></div>%4</div>")
			    .arg(color)
			    .arg(itemIndex + 1)
			    .arg(imgPath)
			    .arg(formatedText);
		...
		if (isImage) {
			return QString(
					   "<div style=\"color : #%1; -qt-block-indent:0; text-indent:0px;\"><div style=\"padding:0px; margin-right:4px;\"><img "
					   "src=\"%2\"></div>%3</div>")
			    .arg(color)
			    .arg(imgPath)
			    .arg(formatedText);
